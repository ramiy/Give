...
class Give_Admin_Settings {
	...
	public static function get_settings_pages() {
		if ( empty( self::$settings ) ) {
			...
			include_once( 'settings/class-give-settings-page.php' );

			$settings[] = include( 'settings/class-give-settings-general.php' );
			...
			switch ( $value['type'] ) {
				...
				case 'single_select_country' :
					...
					?><tr valign="top">
						...
						<td class="forminp"><select name="<?php echo esc_attr( $value['id'] ); ?>" style="<?php echo esc_attr( $value['css'] ); ?>" data-placeholder="<?php esc_attr_e( 'Choose a country&hellip;', 'give' ); ?>" title="<?php esc_attr_e( 'Country', 'give' ) ?>" class="give-enhanced-select">
				...
				case 'multi_select_countries' :
					...
					?><tr valign="top">
						...
						<td class="forminp">
							<select multiple="multiple" name="<?php echo esc_attr( $value['id'] ); ?>[]" style="width:350px" data-placeholder="<?php esc_attr_e( 'Choose countries&hellip;', 'give' ); ?>" title="<?php esc_attr_e( 'Country', 'give' ) ?>" class="give-enhanced-select">
								<?php
									if ( ! empty( $countries ) ) {
...
//			$tooltip_html = give_help_tip( $tooltip_html );
		}
...
*@return bool
	...
	public static function save_fields( $options ) {
		...
		foreach ( $options as $option ) {
			 ...
			 */
			if ( has_action( 'give_update_option_' . sanitize_title( $option['type'] ) ) ) {
				_deprecated_function( 'The give_update_option_X action', '2.4.0', 'give_admin_settings_sanitize_option filter' );
				do_action( 'give_update_option_' . sanitize_title( $option['type'] ), $option );
				continue;
			}
			 ...
			 * @since 2.4.0
			 */
			$value = apply_filters( "give_admin_settings_sanitize_option_$option_name", $value, $option, $raw_value );

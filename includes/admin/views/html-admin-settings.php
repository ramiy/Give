...
<div class="wrap give">
	<form method="<?php echo esc_attr( apply_filters( 'give_settings_form_method_tab_' . $current_tab, 'post' ) ); ?>" id="mainform" action="" enctype="multipart/form-data">
		<nav class="nav-tab-wrapper give-nav-tab-wrapper">
			<?php
				foreach ( $tabs as $name => $label ) {
					echo '<a href="' . admin_url( 'admin.php?page=give-settings&tab=' . $name ) . '" class="nav-tab ' . ( $current_tab == $name ? 'nav-tab-active' : '' ) . '">' . $label . '</a>';
				}
		...
		<?php
			do_action( 'give_sections_' . $current_tab );

			self::show_messages();

			do_action( 'give_settings_' . $current_tab );
		?>
		<p class="submit">

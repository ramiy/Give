...
jQuery(function ($) {
    ...
    $('[id^="give-dedication-option"]').on('change', function () {

        var dedication_option = $(this).prop('checked');
        var this_form = $(this).parents('form');
        var dedication_fields = this_form.find('.give-dedication-fields-wrap');


        //Checked
        if(dedication_option) {
            dedication_fields.show();
        } else {
            dedication_fields.hide();
        }
